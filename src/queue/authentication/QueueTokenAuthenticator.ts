...
import { HeaderConstants, VALID_QUEUE_AUDIENCES } from "../utils/constants";
...
export default class QueueTokenAuthenticator implements IAuthenticator {
  ...
  public async validate(
    ...
    if (
      queueContext.operation === Operation.Queue_GetAccessPolicy ||
      queueContext.operation === Operation.Queue_SetAccessPolicy
    ) {
      this.logger.info(
        `QueueTokenAuthenticator:validate() Operation is not available with OAuth. Skip token authentication.`,
        queueContext.contextID
      );
      return;
    }
  ...
  public async authenticateBasic(
    ...
    const queueContext = context as QueueStorageContext;
    let audMatch = false;
    let m;
    for (const regex of VALID_QUEUE_AUDIENCES) {
      m = regex.exec(aud);
      if (m !== null) {
        if (m[0] === aud) {
    ...
    this.logger.info(
      `QueueTokenAuthenticator:authenticateBasic() Validation against token authentication successfully.`,
      queueContext.contextID
    );
    return true;
  }
